...
function Home() {
  ...
  } = useHasntToken("/api/validate-token");

  const {
    response: data,
    isPending: isPending,
    error: error,
    handleDispatch: loadMessagesDispatch,
  } = useFetch("api/messages", "GET");

  useEffect(() => {
    handleDispatch();
    loadMessagesDispatch();
  }, []);
  ...
  const {
    ...
    error: errorSearch,
    handleDispatch: handleSearch,
  } = useFetch("/api/messages", "POST", { request: search });
      ...
      <div key={message._id} className="mb-6">
        ...
        <div className="w-fit px-3 py-2 bg-orange-100 mb-2">
          ...
          {message.response ? (
                ...
                return (
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: marked.parse(DOMPurify.sanitize(line.text)),
                    }}
                  />
                );
              })}
            ...
            <Error
              styleclass="flex items-center my-2 text-sm text-red-800"
  ...
  const handleSubmit = (e) => {
    ...
    if (search && search.trim() !== "") {
      ...
      console.log("Search message is required");
    }
  };
  ...
  return (
    <main className="h-dvh">
      <Navbar width={headerWidth} handleClearMessages={handleClearMessages} />

      <section
        ref={scrollContainerRef}
        className={`w-full p-3 overflow-y-auto overflow-x-hidden`}
        style={{ height: `calc(100% - ${outputWidth}px)` }}
      >
        {isPending && (
      ...
      <section
        className={`max-w-7xl mx-auto px-3 py-2 border-t border-gray-100`}
        style={{ height: `${searchWidth}px` }}
      >
        <form
        ...
        >
          ...
          {!isPendingSearch && (
            <button
              type="submit"
              className="w-fit ml-2 px-3 py-2 border border-gray-100 bg-blue-500 rounded-md text-white"
            >
              Search
            </button>
          )}
          {isPendingSearch && (
            <button
              disabled
              type="submit"
              className="w-fit flex justify-center items-center ml-2 px-3 py-2 border border-gray-100 bg-gray-900 rounded-md text-white"
            >
              <Spinner width="w-4" height="h-4" />
              <span className="w-fit block ml-2">Loading</span>
            </button>
